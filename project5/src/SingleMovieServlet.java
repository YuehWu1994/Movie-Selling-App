...
public class SingleMovieServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			String query = "SELECT * from (SELECT mc.id, mc.title, mc.year, mc.director, mc.rating, sim.starId  from (SELECT * from movies m LEFT JOIN ratings r ON m.id = r.movieId WHERE m.id = ?) mc LEFT JOIN stars_in_movies as sim ON sim.movieId = mc.id) mId LEFT JOIN stars as s ON s.id = mId.starId;;";
